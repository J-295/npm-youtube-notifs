...
class Notifier extends EventEmitter {
	...
	constructor(dataFileName, newVidCheckInterval) {
		super();
		...
		if (typeof dataFileName === "undefined") {
			dataFileName = "./yt-notifs-data.json"
		} else if (!(
			typeof dataFileName === "string" &&
			dataFileNamePattern.test(dataFileName)
		)) {
			const error = new Error("Invalid dataFileName provided");
	...
	start() {
		...
		fs.stat(this.dataFileName, (err, stat) => {
			if (err === null) {
				fs.readFile(this.dataFileName, "utf8", (err, json) => {
			...
			setTimeout(() => {
				...
				if (!this.ready) {
					const error = new Error("Not starting notifier as it is not ready");
					this.emit("error", error);
					...
					this.checkIntervalId = setInterval(() => {
						...
						if (saveFile) {
							saveFile = false;
							writeFile(this.dataFileName, JSON.stringify(this.data));
						}
